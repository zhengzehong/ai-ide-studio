...
- **项目秘书** — 每个项目可配置多个秘书，选择执行/观察 Agent 和自由 Prompt，按 Cron、Agent 完成或 Task 需处理事件运行；PC/APP 均可查看实时执行历史和失败原因，后台结果按邮箱 Thread 汇总，运行/对话 Session 可直接进入现有工作空间，继续复用 Markdown、工具过程和多文件查看
