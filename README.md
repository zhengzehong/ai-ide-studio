...
- **全局会话坞** — 在同一右侧竖条固定跨项目普通会话，集中查看运行中、未读和最近活动状态，并一键跳转到对应项目 Workspace；固定关系和顺序由服务端保存
...
- **模型档案** — 在设置页维护 New API / OpenAI / Claude 供应商与 Claude Code、Codex 模型档案，可为项目 Agent 绑定不同档案，并按 Claude 档案控制 Agent 是否能主动读取图片
