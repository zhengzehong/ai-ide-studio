...
- **Session 管理** — 会话按项目/Agent 归属展示，支持标题、重命名、复制、本地会话导入、关闭、归档和软删除
...
- **事件中心** — 分类事件收件箱，支持项目作用域事件类别、Agent 写入事件、订阅规则生成待消费记录、消费者 Agent 处理事件并转成任务
...
- **MCP 工具平台** — 提供 `/mcp` HTTP MCP 入口，按 Session token 控制 Agent 可见的 `core.*` / `agent.*` / `team.*` 工具方法；Agent 可通过 `core.model_profile.list` 查看模型档案，并可在 `core.agent.create` 创建 Agent 时绑定模型档案；`team.*` 默认不全局开放，可在工具管理页给 Agent 套用 Team 权限模板或单独开关方法
