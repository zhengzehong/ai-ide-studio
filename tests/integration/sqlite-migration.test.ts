...
describe('SQLite 迁移', () => {
  ...
  test('创建工具上下文、工具调用审计和 schema_migrations 表', () => {
    ...
    expect(migrations).toEqual(['001', '002', '003', '004', '005', '006', '007', '008', '009', '010', '011', '012', '013', '014', '015', '016', '017', '018', '019', '020', '021', '022', '023', '024', '025', '026', '027', '028', '029', '030', '031', '032', '033', '034', '035', '036', '037', '038', '039', '040', '041', '042', '043', '044', '045', '046', '047', '048', '049', '050', '051', '052', '053', '054', '055', '056', '057', '058', '059', '060', '061', '062', '063', '064', '065', '066', '067', '068'])
    ...
    expect(getDb().prepare<[], { name: string }>('PRAGMA table_info(team_members)').all().map(row => row.name)).toContain('model_profile_id')
    ...
    expect(getDb().prepare<[], { name: string }>(`
      SELECT name FROM sqlite_master
      WHERE type = 'index' AND name = 'idx_inspiration_notes_completion'
    `).get()?.name).toBe('idx_inspiration_notes_completion')
    expect(getDb().prepare<[], { name: string }>(`
      SELECT name FROM sqlite_master
