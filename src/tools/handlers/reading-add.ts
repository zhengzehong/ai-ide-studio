...
export const readingAddHandler: ToolHandler = {
  name: 'reading.add',
  description:
    '把当前会话生成的长文加入用户的阅读列表。md/html 必须传 Gateway 所在机器上的绝对文件路径；HTML 的 CSS、JS、图片须放在同目录并使用相对路径。url 只允许 HTTPS。项目、会话和 Agent 由系统自动关联。',
  inputSchema: {
    type: 'object',
...
function resolveSource(format: ReadingItemFormat, content: string): ResolvedSource {
  if (format === 'url') {
    ...
    try {
      ...
      return { error: 'url 必须是有效的 HTTPS 地址' }
    }
    if (url.protocol !== 'https:') return { error: 'url 只允许 HTTPS 地址' }
