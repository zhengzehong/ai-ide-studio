...
export function resolveToolsForSession(agentId?: string, projectId?: string, sessionId?: string): ResolvedTool[] {
  ...
  const allTools = toolStore.list().filter(
    (tool) => tool.enabled
      && isAgentVisiblePlatformTool(tool.name)
      && isToolVisibleForSession(tool.name, sessionId),
  )
  ...
  for (const toolRow of allTools) {
    ...
    const privateTeamTool = def.name.startsWith('team.') && !['team.list', 'team.conversation.list'].includes(def.name)
    const effectiveBinding = privateTeamTool ? agentBinding : agentBinding || projectBinding || globalBinding
