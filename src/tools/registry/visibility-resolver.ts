...
export function resolveVisiblePlatformTools(input: ResolveVisiblePlatformToolsInput): ResolvedTool[] {
  ...
  log.debug({ agentId: input.agentId, projectId: input.projectId, sessionId: input.sessionId, count: resolved.length }, 'resolved visible platform tools')
  return resolved
}

function resolveBindingForTool(toolId: string, bindings: BindingRow[], input: ResolveVisiblePlatformToolsInput): { visible: boolean; binding?: BindingRow } {
...
function rowToBinding(row: BindingRow): ToolBinding {
  return {
    ...
    enabled: row.enabled === 1,
    configOverride: row.config_override_json ? JSON.parse(row.config_override_json) as Record<string, unknown> : undefined,
    createdAt: row.created_at,
  }
}
