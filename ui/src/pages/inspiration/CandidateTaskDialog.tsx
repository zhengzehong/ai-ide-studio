...
interface CandidateTaskDialogProps {
  ...
  busy: boolean
  onClose: () => void
  onConfirm: (input: { title: string; descriptionMarkdown: string; agentId: string; action: CandidateAction }) => Promise<void>
}

export function CandidateTaskDialog({ candidate, action, agents, busy, onClose, onConfirm }: CandidateTaskDialogProps) {
  const [title, setTitle] = useState(candidate.title)
  const [descriptionMarkdown, setDescriptionMarkdown] = useState(candidate.descriptionMarkdown)
  const [agentId, setAgentId] = useState(candidate.suggestedAgentId ?? agents[0]?.id ?? '')
    ...
    <div className="inspiration-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
      <section className="inspiration-task-dialog" role="dialog" aria-modal="true" aria-labelledby="candidate-dialog-title">
        ...
        <div className="inspiration-dialog-body">
          ...
          <label><span>执行 Agent</span><select value={agentId} onChange={(event) => setAgentId(event.target.value)}><option value="">请选择 Agent</option>{agents.map((agent) => <option key={agent.id} value={agent.id}>{agent.name}</option>)}</select></label>
        ...
        <footer>
          ...
          <button type="button" className="inspiration-primary" disabled={!canConfirm} onClick={() => void onConfirm({ title: title.trim(), descriptionMarkdown: descriptionMarkdown.trim(), agentId, action })}>
