...
// 3. 直接对象: {previewId, url, title, target, taskId, createdAt}
...
export function parsePreviewPublishOutput(raw: unknown): PreviewPublishOutput | null {
  ...
  const url = typeof rec.url === 'string' && rec.url ? rec.url : `/preview/${previewId}/`
  const taskId = typeof rec.taskId === 'string' ? rec.taskId : null
  return { previewId, url, title, target, taskId, createdAt }
}

// 把 MCP content 数组 / JSON 字符串 / 已是对象 三种形态统一还原成对象。
function unwrapRawOutput(raw: unknown): unknown {
  if (raw == null) return null
  if (Array.isArray(raw)) {
    // MCP 标准: [{type:'text', text:'<json>'}]。取第一个有 text 的元素。
