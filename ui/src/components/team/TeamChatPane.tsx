...
function mergeProcessItem(current: Record<string, Snapshot>, sessionId: string, item: TurnProcessItemInfo, block: TurnProcessBlock): Record<string, Snapshot> { const snapshot = current[sessionId] || emptySnapshot(sessionId); const existingTurn = snapshot.streaming?.id === item.message_id ? snapshot.streaming : null; const currentTurn = existingTurn || createEmptyTurn(item.message_id); const withStart = withTurnStart(existingTurn, currentTurn); const processBlocks = [...withStart.processBlocks.filter((entry) => entry.id !== block.id), block]; return { ...current, [sessionId]: { ...snapshot, streaming: { ...withStart, processBlocks, senderName: snapshot.senderName, teamAssignment: snapshot.pendingAssignment || withStart.teamAssignment }, running: true } } }
