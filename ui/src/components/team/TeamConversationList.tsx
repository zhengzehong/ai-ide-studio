...
import { Archive, Loader2, Pencil, Plus, RefreshCw, Trash2 } from 'lucide-react'
...
export function TeamConversationList({ team, activeId, onSelect, onMasterSession }: Props) {
  ...
  return <aside style={{ width: 270, borderRight: '1px solid var(--border)', background: 'var(--bg-0)', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
    <header style={{ padding: '12px 12px 8px', display: 'flex', alignItems: 'center', gap: 6 }}><strong style={{ flex: 1 }}>团队会话</strong><button type="button" onClick={() => void load()} disabled={loading} title="刷新" style={iconStyle}>{loading ? <Loader2 size={14} /> : <RefreshCw size={14} />}</button></header>
    <button type="button" onClick={() => void create()} disabled={loading} style={{ margin: '0 10px 8px', padding: '8px', border: '1px dashed var(--blue)', color: 'var(--blue)', background: 'transparent', borderRadius: 7, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}><Plus size={14} /> 新建会话</button>
    ...
    <div style={{ overflowY: 'auto', flex: 1, padding: '0 8px' }}>{loading && items.length === 0 && <div style={{ padding: 12, color: 'var(--text-3)' }}>正在加载...</div>}{!loading && !error && items.length === 0 && <div style={{ padding: 20, textAlign: 'center', color: 'var(--text-3)', fontSize: 12 }}>暂无团队会话</div>}{items.map((item) => <div key={item.id} role="button" tabIndex={0} onKeyDown={(event) => { if (event.key === 'Enter') { onMasterSession(item.master_session_id); onSelect(item) } }} onClick={() => { onMasterSession(item.master_session_id); onSelect(item) }} style={{ padding: '10px', marginBottom: 4, borderRadius: 7, background: activeId === item.id ? 'var(--blue-light)' : 'transparent', cursor: 'pointer' }}><div style={{ display: 'flex', gap: 4, alignItems: 'center' }}><span style={{ width: 7, height: 7, borderRadius: '50%', background: item.status === 'active' ? 'var(--green)' : 'var(--text-3)', flexShrink: 0 }} title={item.status === 'active' ? '进行中' : '已归档'} /><b style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.title}</b><button type="button" title="重命名" onClick={(event) => { event.stopPropagation(); void rename(item) }} style={iconStyle}><Pencil size={13} /></button><button type="button" title="归档" onClick={(event) => { event.stopPropagation(); void archive(item) }} style={iconStyle}><Archive size={13} /></button><button type="button" title="删除" onClick={(event) => { event.stopPropagation(); void remove(item) }} style={iconStyle}><Trash2 size={13} /></button></div><div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 4 }}>{formatTime(item.updated_at)}</div></div>)}</div>
  </aside>
}
