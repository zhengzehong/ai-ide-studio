...
export function SessionDockDrawer() {
  ...
  if (pickerOpen && open) return <section className="session-dock-drawer session-dock-drawer--open"><SessionDockPicker /></section>
    ...
    >
      <header className="session-dock-header">
        <div className="session-dock-heading">
          <strong>置顶会话</strong>
          <small>{items.length} 个置顶 · {runningCount} 个运行 · {unreadCount} 个未读</small>
        </div>
