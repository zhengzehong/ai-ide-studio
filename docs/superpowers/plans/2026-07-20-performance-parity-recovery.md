...
- [x] Add a PC shell regression test proving global assistant/page state remains mounted during temporary disconnect.
...
- [x] Add ws-client tests proving listeners are ready before subscribe/resume and reconnect cannot race listener installation.
- [x] Add PC resync tests that resolve the Session owner project and refresh that project's Session/messages/events/cache even when inactive.
- [x] Register application listeners for the lifetime of the application shell rather than per connection.
- [x] Add mobile shell tests proving a transient disconnect preserves current page, selected Session, messages, and draft.
- [x] Keep reconnect controls visible without replacing the active application tree; explicit logout remains unchanged.
- [x] Run PC ws-client/bootstrap/project-scope and mobile connection/session tests.
- [x] Commit Batch 4 and report a task milestone.
