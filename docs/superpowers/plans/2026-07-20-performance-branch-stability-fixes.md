...
- [x] **Step 1: Write the failing real-contention test**
...
- [x] **Step 3: Acquire the Writer transaction immediately**
...
- [x] **Step 4: Run the Writer tests and verify GREEN**
...
- [x] **Step 5: Commit the SQLite fix**
...
- [x] **Step 1: Write failing dispatcher lane tests**
...
- [x] **Step 3: Introduce per-command session lanes**
...
- [x] **Step 4: Register pending interactions before publishing**
...
- [x] **Step 5: Run dispatcher and interaction tests**
...
- [x] **Step 6: Commit the command fix**
...
- [x] **Step 3: Implement and share the resolver**
...
- [x] **Step 4: Run Runtime preference tests and verify GREEN**
...
- [x] **Step 5: Commit the Runtime mode fix**
...
- [x] **Step 1: Add guarded launcher configuration**
...
- [x] **Step 2: Add startup documentation**
...
- [x] **Step 3: Validate the launcher guard**
...
- [x] **Step 2: Run mandatory repository verification**
...
- [x] **Step 3: Start the isolated instance**
...
- [x] **Step 4: Update this checklist and commit evidence**

Mark completed checkboxes, record verification totals, then commit the plan update.

- [x] **Step 5: Request code review without merging PRD**
...
- SQLite RED: the contention test failed with `database is locked` in 39ms while the API lock was still held.
- SQLite GREEN: the Writer waited about 108ms and committed after the API lock was released.
- Command RED: permission and cancel commands did not enter the executor while prompt remained unresolved.
- Command GREEN: permission and cancel completed while prompt was still blocked; same-session prompts remained ordered.
- Interaction RED/GREEN: an immediate permission response changed from `resolvedDuringPublish=false` to a resolved ACP response.
- Targeted regression: 7 files, 29 tests passed.
- Full suite: 246 files, 1199 tests passed.
- `npm run build`: passed for server, PC, and mobile.
- `npm run lint`: passed for server, PC, and mobile.
- UI bundle gate: entry 396.26 KiB, 18 dynamic routes.
- Isolated launch: HTTP 200 on `http://127.0.0.1:19000/`; same-origin Realtime at `/realtime`; database at `data-perf/ai-ide.sqlite`.
- PRD remained on `eb9ce6926cfe593df654d9b72c832474554d197f` and its `18900` listener was not changed.
- Code review: APPROVED, event `perf-stability-r1`, range `dbead4d..992bab6`, no P0/P1 findings.
