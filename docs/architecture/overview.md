...
项目灵感工作台是 PC 端的项目级记录与任务候选入口。每个项目维护一个当前长期灵感 Session，用户保存 Markdown 和图片原文后，后台把整理请求串行投递到这个 Session，使持续讨论和自动整理共享上下文。Agent 必须通过仅在该 Session 可见的 `inspiration.analysis.publish` 发布摘要、Markdown 方案、待确认问题和候选任务；该 Session 隐藏平台任务创建、步骤编排和 Schedule 变更工具，不能绕过人工确认直接派发。原文和整理结果分别持久化，revision CAS 阻止旧回复覆盖新编辑；候选确认后才复用标准 Task/Step/Session 链路执行。
