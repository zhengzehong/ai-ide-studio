...
APP 团队会话使用独立的 `MobileTeamChatSurface` 展示布局，按普通 `ChatPage` 的样式和操作组织，两者继续共用气泡、正文与输入基础组件。团队数据由 `TeamChatPane` 聚合各成员 Session；`team-chat-refresh` 控制成员完成增量刷新与恢复版本，`team-read-control` 协调自动已读和用户标记未读。初次加载限制成员查询并发，历史工具详情按需读取，成员完成只更新对应来源消息。

WS 恢复入口 `gateway/rpc/session-recovery` 复用 QueryPort 的轻量状态查询；`queries/turn-event-page` 对当前轮事件按字节和条数分页。恢复时先重新接收实时事件，再按快照序号重放期间接收的内容；实时进程队列区分可恢复增量与必须有确定结果的控制通信，控制消息不会在恢复屏障内静默丢失。
...
桌面发布校验器 `scripts/electron-package-integrity.mjs` 对照编译目录与实际发布目录，校验主进程、嵌套模块和 preload 的文件内容及静态相对依赖；后端启动文件按 extraResources 中的最终位置校验。校验失败阻止替换旧发布包。Windows 安装版使用带目录选择的 NSIS 向导，便携版保持直接运行。
