...
| `tasks.page` | `{ projectId?, status?, query?, createdFrom?, createdBefore?, excludeTerminal?, limit?, cursor? }` | `{ items, total, hasMore, nextCursor }` | Workspace 使用的游标分页读取。默认 50、最大 200；按 `created_at DESC, id DESC` 排序，`cursor` 是上一页最后一个 Task ID。旧 `tasks.list` 保持完整数组返回，供兼容入口使用。 |
