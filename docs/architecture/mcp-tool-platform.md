...
任务和定时工具的会话策略按创建/分派拆分：`studio.task.create` 只接收 `title/description/selfExecute/projectId`，不接收会话策略；动态分派通过 `studio.task.assign` 传 `sessionMode/sessionId`。兼容工具 `core.task.create` / `create_task` 仍可在创建时携带 `assignAgentId + sessionMode/sessionId`；前端 `rules.*` RPC 和规则引擎仍支持通用的 `create_task` / `send_prompt`。AI 可见的 `studio.schedule.create` 只为当前 Agent 的当前 Session 创建 `send_prompt` 规则，目标和 `session_mode=existing` 由 ToolContext 注入，不接受模型指定的 Agent、Session 或任务类型；旧的 `create_schedule` 不再 seed 为 AI 工具，已有记录会在启动时清理；`studio.schedule.update` 等管理工具继续操作已有规则。`existing` 复用指定会话，`new_each` 每次新建，`new_fixed` 在可持久化的规则或订阅上首次新建后固定复用。
...
`files.present` 用于把 Agent 已完成的文件作为会话交付物展示给用户。调用参数包含展示标题和 1-20 个文件路径；相对路径按当前项目工作空间解析，显式绝对路径可以指向服务端账号可读取的任意文件。工具上下文提供可信的 `projectId` 和项目工作目录，handler 只校验并读取文件元数据，不把文件正文写入工具结果。
